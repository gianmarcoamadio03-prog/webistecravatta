...
// CONFIG
// =====================
const VERSION = "2026-03-23 | weidian -> official sheet v1";
...
async function extractItemData(page, url) {
  ...
  const data = await page.evaluate(() => {
    ...
    function pushImage(arr, src, bucket, order, area = 0) {
      ...
      const bad = ["logo", "icon", "avatar", "coupon", "qrcode", "qr"];
    ...
    const textCandidates = Array.from(
      ...
      .map((el) => {
        ...
        if (rect.top < 120 || rect.top > 1800) return null;
    ...
    const title = textCandidates[0]?.text || document.title || "";
...
function makeRow(job, itemUrl, itemData) {
  ...
  return [
    ...
    "ok",                  // P status
    itemUrl,               // Q source_url  <-- usato per dedupe/update
    "",                    // R source/shop extra (vuoto per Weidian)
